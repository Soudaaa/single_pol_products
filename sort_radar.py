...
def main(radar = None):
    ...
    for fields in list(radar.fields.keys()):
        ...
        radar.fields[fields]['data'] = np.ma.vstack(field_final).astype(np.float32)
    
    # overwrite the azimuth, elevation and number of rays arrays
    radar.azimuth['data'] = az_final.flatten().astype(np.float32)
    radar.elevation['data'] = ele_final.flatten().astype(np.float32)
